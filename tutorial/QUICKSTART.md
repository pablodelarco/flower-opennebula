...
> **Note:** The dashboard reads cluster state from OpenNebula CLI and Docker container logs. It requires SSH access to each VM, which is already configured if you completed the previous steps.
...
**Monitor production workloads** -- For Prometheus-based monitoring, add `prometheus_client` gauges to your ServerApp strategy or use container-level metrics via cAdvisor.
See [`spec/12-monitoring-observability.md`](../spec/12-monitoring-observability.md).
