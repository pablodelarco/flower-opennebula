...
**Scope:** All variables defined in Phase 1 (base appliance architecture), plus placeholder variables for Phase 2 (TLS), Phase 3 (ML frameworks), and Phase 6 (GPU). Placeholder variables are documented here for completeness but are not functional in Phase 1.
...
|----------|-------|-----------|
| SuperLink parameters | 11 | SuperLink only |
| SuperNode parameters | 7 | SuperNode only |
...
These 11 variables configure the Flower SuperLink appliance. All are optional. Zero-config deployment works with all defaults (see Section 9).
...
| 3 | `FL_STRATEGY` | `O\|list\|Aggregation strategy\|FedAvg,FedProx,FedAdam\|FedAvg` | list | `FedAvg` | One of: `FedAvg`, `FedProx`, `FedAdam` | ServerApp config: `strategy` class name |
...
| `FL_STRATEGY` | Exact match: `FedAvg`, `FedProx`, or `FedAdam` | `"Unknown FL_STRATEGY: '${VALUE}'. Valid options: FedAvg, FedProx, FedAdam."` |
